...
fit_binomial_null <- function(tick_data, has_suppressed = TRUE, no_numeric = FALSE, town_name, n.iter = 91000, data_list, init_detection_rate,init_ic ){
  ...
  numeric_indices <- which((tick_data$Number != "<6" )|(is.na(tick_data$Number)))
  numeric_indices <- numeric_indices[numeric_indices != 1]
  na_indices <- which(is.na(tick_data$Number))
  ...
  data_list$na_indices <- na_indices
  ...
  ## Check if has ONLY suppressed data.
  if (rlang::is_empty(numeric_indices)){
   no_numeric <- TRUE
  }
  if(no_numeric){
    print("Entering No Numeric")
    RandomWalk = "
model{
  #### Process Model
   for( i in less_than_six_indices){
    dummy[i] <- probability[i - 1] + yearly_error 
    probability[i] <- max(dummy[i], 1.622593e-10) 
    y[i] ~ dbinom(probability[i], pop[i]) T(1,5)
   }
...
"
  }else if(has_suppressed){
    print("Entering Has Suppressed")
    RandomWalk = "
model{
...
"
    ...
    print("Entering Standard Process model")
        RandomWalk = "
model{
...
init <- list()
for(i in 1:nchain){
  init[[i]] <- list() # from 2018 Maine rate of lyme # probability[1:10] <- rep(0.001324, 10) https://www.maine.gov/dhhs/reports/2018/Lyme-and-Other-Tickborn-Illnesses-2018-Report.pdf
}
...
if(is.na(burnin)){
  ...
  file_name <- paste("/Users/tess/Documents/work/Maine_Tick_Disease_Forecast/Jags_output/", date_stamp, ".", "binomial_simple_trash",".",town_name,".", did_it_converge, ".","JAGS_run.Rdata", sep = "")
