...
for ( i in seq_along(towns_to_run)){
  ...
  town_x_ic$rate <- 1/town_x_ic$x_ic[i]
...
model{
  ...
  #### Process Model
   for (i in 2:n){
    ...
    probability[i] <- probability[i - 1] + yearly_error
     dummy[i] <- max(probability[i], 1.622593e-05) 
    y[i] ~ dbinom(dummy[i], pop[i]) T(1,5)
  }

  ## Prior
  probability[1] ~ dexp(rate) #uniform probability
...
model{
  ...
  #### Process Model
   for (i in 2:n){
    ...
    probability[i] <- probability[i - 1] + yearly_error # Watch out for numeric errors
    dummy[i] <- max(probability[i], 1.622593e-05) # probability can get very small but can't go negative or to zero
    y[i] ~ dbinom(dummy[i], pop[i])
  }
...
data_list <- list(y = tick_disease_tmp$Number, 
                  ...
                  mean_error = mean(town_x_ic$x_ic)/9, 
                  var_error = 0.0003, 
                  pop = as.numeric(tick_disease_tmp$Population))

print(town_name)
output <- fit_binomial_null(tick_data = tick_disease_tmp, has_suppressed = TRUE, town_name, n.iter = 1000, data_list)
