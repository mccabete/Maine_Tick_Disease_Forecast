...
for (i in seq_along(towns_to_run)){
  ...
  town_x_ic$x_ic[i] <- rgamma(1, obs_ticks, 1/10)# Need to redo for normal data model. 
...
model{
  ...
  for(t in numeric_indices){
   ...
   y[t] ~ dnorm(x[t], 1/observation_error) T(0,)
  ...
  for(t in less_than_six_indices){   
   ...
   y[t] ~ dnorm(x[t], 1/observation_error) T(rate_cutoff_low[t], rate_cutoff_high[t])
  }
  ...
  #### Priors
  observation_error ~ dnorm(10, 1/10)
...
jags.out   <- coda.samples (model = j.model,
                            variable.names = c( "x", "observation_error", "yearly_variability" ),
                                n.iter = n.iter)
...
print(town_name)
output <- fit_rate_random_walk(tick_disease_tmp, has_suppressed = TRUE, town_name, n.iter = 100000, data_list )
