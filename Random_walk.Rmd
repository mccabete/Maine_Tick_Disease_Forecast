...
model{
  ...
  for(t in numeric_indices){
   ...
   w[t] <- x[t] / pop[t] * 100000 ## Change to rate
 }
...
model{
 ...
 #### Data Model
  ...
  w[1] <- (x[1] / pop[1]) * 100000 # Initial rate
...
data_list <- list(less_than_six_indices = points_of_less_than_six, numeric_indices =numeric_indices, n = 7, a_obs = beta_of_true_tick_rate$alpha, b_obs = beta_of_true_tick_rate$beta , y = tick_data$Number,  x_ic = town_x_ic$x_ic[town_x_ic$Location == town_name] ,a_gam = a_gam, b_gam = b_gam, pop = as.numeric(tick_data$Population))

init_data <- as.numeric(tick_data$Number) + 5
print(town_name)
freeport <- fit_random_walk(tick_data, has_suppressed = TRUE, town_name, n.iter = 1000, data_list, init_detection_rate = 0.9999, init_ic = val_2008 + 2, init_data )
