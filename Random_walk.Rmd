...
```{r}
fit_random_walk <- function(tick_data, has_suppressed = TRUE, town_name, n.iter = 3000, data_list, init_detection_rate,init_ic, init_data ){
...
model{
  ...
  for(t in 2:n){
    a[t] <- (x[t-1]^2)/S
    b[t] <- x[t-1]/S
    lambda[t] ~ dgamma(a[t], b[t])
    x[t] ~ dpois(lambda[t])
  }
  
  #### Priors
  tau ~ dgamma(a_gam, b_gam)
  x[1] ~ dpois(x_ic) 
  lambda[1] ~ dgamma(a_gam, b_gam)
...
jags.out   <- coda.samples (model = j.model,
                            variable.names = c( "x", "detection_rate"),
                                n.iter = n.iter)
...
for (i in seq_along(towns_to_run)){
  ...
  #towns_failed_to_run <- c("South_Portland", "Harpswell",  "Brunswick", "Pownal",  "Portland", "Frye_Island") 
...
data_list <- list(less_than_six_indices = points_of_less_than_six, numeric_indices =numeric_indices, n = 7, a_obs = beta_of_true_tick_rate$alpha, b_obs = beta_of_true_tick_rate$beta , y = tick_data$Number,  x_ic = town_x_ic$x_ic[town_x_ic$Location == town_name] ,a_gam = a_gam, b_gam = b_gam)

init_data <- as.numeric(tick_data$Number) + 5
print(town_name)
run_output[[i]] <- try(fit_random_walk(tick_data, has_suppressed = TRUE, town_name, n.iter = 1000, data_list, init_detection_rate = 0.9999, init_ic = val_2008 + 5, init_data ))
