...
This paper reports a "True positive rate" of the number of ticks sent for testing. Here I solve for a "false negative rate" and the range that is reported. 
```{r}
...
# Solving for total submited specimines 
num_tested <- 288000/0.12
percent_observed_test_1 <- 5.76 # For Two-tiered whole cell 
percebt_observed_test_2 <- 11.89 # EIA/IFA stand-alone or first tier

false_negative_max <- -1*(5.76 - 18.5) # note I might not even need false negatives. binomial is listed as probability of "sucess" so a "true positive", which the study reports directly
false_negative_min <- -1*(5.76 - 10 )
#false_negative_min_test_2 <- -1*(11.98 - 10) # note: only applicable for one test. Not sure which is used in Maine, and likely only one? Data from paper suggested that top one more common. 
...
mu_true_pos <- 0.12
var_true_pos <- 0.03 # Somethign larger than 0.1 and 0.18, but still constrained by paper
...
[This estimate](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4550147/) of 2005-2010 national incidence rate useing health insurence claims also argues for large number of underreporting. 
...
# For binom error
data_list <- list(less_than_six_indices = points_of_less_than_six, numeric_indices =numeric_indices, n = 10, a_obs = beta_of_true_tick_rate$alpha, b_obs = beta_of_true_tick_rate$beta , a_add = 2, r_add = 0.5,  y = as.numeric(tick_disease_tmp$Number),  x_ic = 5, tau_ic = 1)
...
fit_random_walk <- function(tick_data, has_suppressed = TRUE, town_name, n.iter = 11000, data_list){
  ...
  tick_disease_tmp <- dplyr::arrange(tick_disease_tmp, by = tick_disease_tmp$Number)
...
model{
  ...
  for(t in numeric_indices){
   ...
   y[t] ~ dbinom(false_negative, x[t])
 }
 
  for(t in less_than_six_indices){   ## Maybe consider a population cutoff, and allow zeros
   y[t] ~ dbinom(false_negative, x[t]) T(1,5)
  }

  #### Process Model
  for(t in 2:n){
    x[t] ~ dpois(x[t-1])#,pop[t]) 
  }
  
  #### Priors
  x[1] ~ dpois(x_ic) #,tau_ic) # Could get from: non-cumberland data, 2007 data, canadian data
  false_nagative ~ dbeta(a_obs,b_obs) 
...
model{
  ...
  #### Process Model
  for(t in 2:n){
    x[t] ~ dpois(x[t-1])#, pop[t]) #,tau_add)
  }
  
  #### Priors
  x[1] ~ dpois(x_ic)#,tau_ic) # Could get from: non-cumberland data, 2007 data, canadian data
  false_negative ~ dbeta(a_obs, b_obs) # Tied to the false positive rate in 
  #tau_add ~  dgamma(a_add,r_add) #could estimate from: non-cumberland data, years we aren't useing, new england data. 
...
jags.out   <- coda.samples (model = j.model,
                            variable.names = c( "tau_add", "x", "false_negative"),
                                n.iter = n.iter)
...
Some notes on different models we tried: 
- A binomial data model where false_negative 