...
for (i in seq_along(towns_to_run)){
  ...
  town_x_ic$x_ic[i] <- (rnbinom(1, obs_ticks, 0.12) +obs_ticks) # add number of successes to number of failure to get total number of trials
}


```

The negative binomial returns the number of failures that occur before a certain number of successes, where the probability of success is 0.12. We are looking at observations which we are presuming are the result of a binomial process where the number of trials is the true number of lyme disease incidents. To get back that number of trials from observations, we need to get the number of failures (the result of a random draw from the negative binomial) and add that to the number of sucesses (The observations). 

Note on the use of negative binomial: Here we use a phraseing of the negative binomial representing ["number of trials where k sucesses is known"](https://en.wikipedia.org/wiki/Negative_binomial_distribution). 
