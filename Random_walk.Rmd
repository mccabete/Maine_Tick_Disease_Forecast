...
# For binom error
data_list <- list(less_than_six_indices = points_of_less_than_six, numeric_indices =numeric_indices, n = 7, a_obs = beta_of_true_tick_rate$alpha, b_obs = beta_of_true_tick_rate$beta , y = tick_disease_tmp$Number,  x_ic = town_x_ic$x_ic[town_x_ic$Location == town_name] ,yearly_error = mean_year_year_variability)
...
init <- list()
for(i in 1:nchain){
  init[[i]] <- list( detection_rate <- 0.2 , x_ic <- 5) # Maybe check  these 
}
