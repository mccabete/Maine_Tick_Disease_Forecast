...
for ( i in seq_along(towns_to_run)){
  ...
  params <- estBetaParams(town_x_ic$x_ic[i],town_x_ic$x_ic[i]/1.01 ) #Results in about an order-of-magnitude variability
...
if(is.na(burnin)){
  ...
  file_name <- paste("/Users/tess/Documents/work/Maine_Tick_Disease_Forecast/Jags_output/", date_stamp, ".", "binomial_null_with_prior",".",town_name,".", did_it_converge, ".","JAGS_run.Rdata", sep = "")
...
# probability at infinate density "Chebeague_Island",
# Error in node probability Invalid parent values "Long_Island"
# Also got at " Error in node probability Slicer stuck at value with infinite density" for "Freeport"


towns_to_run <-  c("Harpswell", "Freeport", "Brunswick", "Pownal",  "Yarmouth", 
                         "Cumberland", "Standish",  "Portland", "Falmouth", "South_Portland", 
                         "Cape_Elizabeth",  "Scarborough", "Harrison", "Gray", "North_Yarmouth", 
...
print(town_name)
output <- fit_binomial_null(tick_data, has_suppressed = TRUE, town_name, n.iter = 199000, data_list, init_detection_rate = 0.9, init_ic = as.numeric(tick_data$Number[1]) )
