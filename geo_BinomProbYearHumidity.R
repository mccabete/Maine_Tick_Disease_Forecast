...
fit_binomial_prob_year <- function(tick_data, has_suppressed = TRUE, town_name, n.iter = 900000, data_list, init_detection_rate, init_data){
  ...
  if(has_suppressed){
    ...
    logit(probability[t]) <- beta[1] + beta[2]*humidity[t]
  ...
  n.iter <- n.iter
  iter2save <- 5000
  variable.names <- c( "x", "detection_rate", "beta")
  ...
  file_name <- paste("/Users/tess/Documents/work/Maine_Tick_Disease_Forecast/Jags_output/", date_stamp, ".", "binomial_prob_year_humidity",".",town_name,".", did_it_converge, ".","JAGS_run.Rdata", sep = "")
  # file_name <- paste("/Jags_output/", date_stamp, ".", "binomial_prob_year_humidity",".",town_name,".", did_it_converge, ".","JAGS_run.Rdata", sep = "")
...
tick_disease_tmp[,3][tick_disease_tmp[,2] > 2016] <- NA # Hold out three years for validati
...
# humidity <- as.vector(aggrigate_data("/usr3/graduate/tmccabe/Maine_Tick_Disease_Forecast/" ,"Humidity", town= town_name)$year.mean)
humidity <- as.vector(aggrigate_data("","Humidity", town= town_name)$year.mean)
